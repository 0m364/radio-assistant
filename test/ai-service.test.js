...
async function runTests() {
    ...
    try {
        ...
        AIService.sendPrompt = async () => '```json\n{"type": "Military", "urgency": "IMMEDIATE", "cipher_status": "CLEAR", "summary": "Alert", "keywords": []}\n```';
        result = await AIService.analyzeTraffic('Some text');
        assert.strictEqual(result.type, 'Military', "Should extract JSON from markdown");
        assert.strictEqual(result.urgency, 'IMMEDIATE', "Should extract JSON from markdown");
        ...
        // Mock sendPrompt to return invalid JSON
        ...
        assert.ok(result.summary.includes('Analysis Failed'), "Summary should indicate failure");
